...
# bool, int, string, list, set, dict, tuple, ndarray, pd.series,
...
df.groupby(["Sex", "Embarked", "Pclass"]).agg({"PassengerId": "count", "Age": ["min", "max", "mean"]})
...
def cat_summary(dataframe, col_name, plot=False):
    ...
    print("##########################################")
    if plot:
        sns.countplot(x=dataframe[col_name], data=dataframe ,hue=dataframe["Pclass"])
        plt.show()
...
# örnek olsun diye gösterildi.
def num_summary(dataframe, numerical_col, plot=False, plot_type=False):
    quantiles = [0.05, 0.10, 0.25, 0.50, 0.75, 0.80, 0.90, 0.95, 0.99]
    print(dataframe[numerical_col].describe(quantiles).T)
    if plot:
        if plot_type:
...
num_summary(df, "Age", plot=True, plot_type=True)
...
num_summary(df, "Age", plot=True, plot_type="box_plot")
...
for col in num_cols:
    target_summary_with_num(dff, "TARGET", col)

for col in cat_cols:
    target_summary_with_cat(dff, "TARGET", col)
