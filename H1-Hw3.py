...
[columns.upper() for columns in df.columns ]


[(F"{columns}_FLAG").upper() for columns in df.columns if "no" not in columns ]
...
[columns for columns in df.columns if df[F"{columns}"].dtype != "O"]
